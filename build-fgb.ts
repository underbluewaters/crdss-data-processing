...
const ds = driver.create('./output/cells.fgb');
...
// Create a read stream from the joined-cells.csv file
const stream = createReadStream("./output/joined-cells.csv");
let i = 0;
console.log('parse');
Papa.parse<{ id: string } & any>(stream, {
  ...
  complete: () => {
    ds.close();
    console.log(`Wrote ${i.toLocaleString()} cells to output/cells.fgb`);
  },
  error: (err) => {
    console.error(err);
  }
